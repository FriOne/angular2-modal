...
function prepareCommands(pluginName) {
  ...
  return {
    ts: `tsc --out ${path.join(config.PATHS.dist.bundles, filename)} --target es5 --allowJs ${path.join(config.PATHS.tmp, filename)}`,
    rollup: `rollup -c ${path.join(rollupRoot, ROLLUP_CONFIG)}`
  }
}
